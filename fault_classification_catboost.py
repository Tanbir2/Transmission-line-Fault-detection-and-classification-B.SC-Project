#!/usr/bin/env python3
"""
Transmission Line Fault CLASSIFICATION (multiclass) using CatBoost.

Usage:
  python fault_classification_catboost.py --csv "Classification-Unbalance-Without-Noise.csv" --plot
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Fault classification with CatBoost (multiclass).")
    p.add_argument(
        "--csv",
        required=True,
        help="Path to classification CSV (must contain faultType column).",
    )
...
def main() -> int:
    ...
    if "faultType" not in df.columns:
        raise ValueError("CSV must contain a 'faultType' column for multiclass labels.")

    # As in the appendix: drop these columns if they exist
    drop_cols = [c for c in ["A", "B", "C", "G", "faultType"] if c in df.columns]
